# Contributing to DataCops Consent Manager
...
**Note:** Playground uses a shared hardcoded KickscraperTool API token.
...
**Important**: Bot detection, crawling, and scraping techniques are continuously evolving. Before filing a new issue or discussing detection techniques publicly, consider whether the topic is suitable for the general audience. Please discuss sensitive topics directly with [support@kickscraper.com](mailto:support@kickscraper.com) with the repository maintainers.

One can create a new signal source in this client-side library, however, it needs to be also accepted and processed on the backend side properly. The recommended approach is to [file an issue in the repository](https://github.com/shakiltousif/kickscraper-tool/issues) and discuss signal details with the maintainers.
...
6. Describe the version changes in the [releases section](https://github.com/shakiltousif/kickscraper-tool/releases).
